...
export default function Request(props: Props) {
    ...
    const handleDropBl = acceptedFiles => {
        acceptedFiles.map(file => {
            let fd = new FormData();
            setBlFileName(file.name);
            fd.append("Type", "BL");
            fd.append("File", file);
            fd.append("FileName", file.name);
            setBlFileShowLoading(true);
            DeliveryOrder.uploadDocument(fd)
            .then(data => {
                setBlFileShowLoading(false);
    ...
    const handleDropLetterIndemnity = acceptedFiles => {
        acceptedFiles.map(file => {
            let fd = new FormData();
            setLetterIndemnityFileName(file.name);
            fd.append("Type", "LainLain");
            fd.append("File", file);
            fd.append("FileName", file.name);
            setLetterIndemnityFileShowLoading(true);
            DeliveryOrder.uploadDocument(fd)
            .then(data => {
                setLetterIndemnityFileShowLoading(false);
                window.console.log(data);
            })
            .catch(error => {
                setLetterIndemnityFileShowLoading(false);
    ...
    const handleDropSuratKontainer = acceptedFiles => {
        acceptedFiles.map(file => {
            setSuratKontainerFileName(file.name);
            let fd = new FormData();
            fd.append("Type", "LainLain");
            fd.append("File", file);
            fd.append("FileName", file.name);
            setSuratKontainerFileShowLoading(true);
            DeliveryOrder.uploadDocument(fd)
            .then(data => {
                setSuratKontainerFileShowLoading(false);
                window.console.log(data);
            })
            .catch(error => {
                setSuratKontainerFileShowLoading(false);
    ...
    const columnDefs = [
        ...
        { title: t(translations.entities.general.grossWeight) },
            ...
            <div>
                        ...
                        >
                            ...
                            >
                                <HeaderItem
                                    ...
                                    description={props.data.notifyPartyName}
                                />
                            </div>
                        </Col>
                        <Col md="6">
                        ...
                        <Col xs="12">
                            <Row>
                                ...
                                <Col xs="12" lg="6">
                                    ...
                                    <Dropfile
                                        hideRemoveButton={true}
                                        onDrop={handleDropLetterIndemnity}
                                        showLoading={
                                            letterIndemnityFileShowLoading
                                        }
                                        isPreview={true}
                            ...
                            <Row className="mt-4">
                                ...
                                >
                                    ...
                                    <Dropfile
                                        hideRemoveButton={true}
                                        onDrop={handleDropSuratKontainer}
                                        showLoading={
                                            suratKontainerFileShowLoading
                                        }
                                        isPreview={true}
                ...
                {props.showActionButton && (
                    <div className="my-4 pb-5 d-flex justify-content-end">
                        ...
                        />
                        <GologsButton
                            content={<GeneralTranslation slug="confirm" />}
                            variant="secondary"
