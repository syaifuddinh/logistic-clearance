...
import BusinessType from "../../../../../endpoints/Master/BusinessType";
...
export default class List extends React.Component {
    ...
    onSubmit = async (id = null) => {
        ...
        try {
            if (id) {
                dt = await BusinessType.update(this.state.data, id);
            } else {
                dt = await BusinessType.store(this.state.data);
            }
            this.setState({data: {}})
            this.refreshTable();
        } catch(e) {
    ...
    show = async (id) => {
        ...
        this.setState({data: data});
        try {
            dt = await BusinessType.show(id);
            data.businessName = dt.businessName;
            data.businessCode = dt.businessCode;
            this.setState({data: data});
        } catch(e) {
            this.showErrorDefaultMessage();
        }
    ...
    componentDidMount() {
        ...
        breadcrumbs = [
            ...
            {
                name: <GeneralTranslation slug="sidebarMenu.businessType" />
            }
        ];
        this.setState({breadcrumbs: breadcrumbs});

        table.columnDefs = [
            { title: <GeneralTranslation slug="businessCode" />},
            { title: <GeneralTranslation slug="businessName" />},
            { title: ""}
        ];
                        ...
                        <Col className="px-3 pt-4  d-flex justify-content-between align-items-center">
                            <div className="d-inline-block">
                                <h4 className="font-weight-bold d-inline-block mb-0 text-capitalize">
                                    <GeneralTranslation slug="sidebarMenu.businessType" />
                                </h4>
