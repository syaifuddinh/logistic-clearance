...
import Module from "../../../../../endpoints/Master/Module";
...
export default class List extends React.Component {
    ...
    onSubmit = async (id = null) => {
        ...
        try {
            ...
            if (id) {
                dt = await Module.update(this.state.data, id);
            } else {
                dt = await Module.store(this.state.data);
            }
            this.setState({data: {}})
            this.refreshTable();
        } catch(e) {
    ...
    show = async (id) => {
        ...
        this.setState({data: data});
        try {
            dt = await Module.show(id);
            data.moduleCode = dt.moduleCode;
            data.moduleName = dt.moduleName;
            this.setState({data: data});
        } catch(e) {
            this.showErrorDefaultMessage();
        }
    ...
    componentDidMount() {
        ...
        breadcrumbs = [
            ...
            {
                name: <GeneralTranslation slug="sidebarMenu.modules" />
            }
        ];
        this.setState({breadcrumbs: breadcrumbs});

        table.columnDefs = [
            { title: <GeneralTranslation slug="modulesCode" />},
            { title: <GeneralTranslation slug="modulesName" />},
            { title: ""}
        ];
                        ...
                        <Col className="px-3 pt-4  d-flex justify-content-between align-items-center">
                            <div className="d-inline-block">
                                <h4 className="font-weight-bold d-inline-block mb-0 text-capitalize">
                                    <GeneralTranslation slug="sidebarMenu.modules" />
                                </h4>
